function stats = grayrlpropsSimple(GLRLM, StatsName)
...
for p = 1 : numGLRLM
    ...
    %
    %------------------------Statistics-------------------------------
    if length(StatsName) == 2
        if StatsName == 'RP'
            stats(p,:)= N_runs/N_tGLRLM;
        end
    elseif length(StatsName) == 3
        if StatsName == 'SRE'
            stats(p,:)= sum(p_r./(c_vector.^2))/N_runs;
        elseif StatsName == 'LRE'
            stats(p,:)= sum(p_r.*(c_vector.^2))/N_runs;
        elseif StatsName == 'GLN'
            stats(p,:)= sum(p_g.^2)/N_runs;
        elseif StatsName == 'RLN'
            stats(p,:)= sum(p_r.^2)/N_runs;
        end
    elseif length(StatsName) == 4
        if StatsName == 'LGRE'
            stats(p,:)= sum(p_g./(r_vector.^2))/N_runs;
        elseif StatsName == 'HGRE'
            stats(p,:)= sum(p_g.*r_vector.^2)/N_runs;
        elseif StatsName == 'GLNU'
            stats(p,:)= calculate_GLNU(tGLRLM,r_matrix,c_matrix,N_runs);
        elseif StatsName == 'RLNU'
            stats(p,:)= calculate_RLNU(tGLRLM,r_matrix,c_matrix,N_runs);
        end
    elseif length(StatsName) == 5
        if StatsName == 'SGLGE'
            stats(p,:)= calculate_SGLGE(tGLRLM,r_matrix',c_matrix',N_runs);
        elseif StatsName == 'SRHGE'
            stats(p,:)= calculate_SRHGE(tGLRLM,r_matrix',c_matrix',N_runs);
        elseif StatsName == 'LRLGE'
            stats(p,:)= calculate_LRLGE(tGLRLM,r_matrix',c_matrix',N_runs);
        elseif StatsName == 'LRHGE'
            stats(p,:)= calculate_LRHGE(tGLRLM,r_matrix',c_matrix',N_runs);
        elseif StatsName == 'SRLGE'
            stats(p,:)= calculate_SRLGE(tGLRLM,r_matrix',c_matrix',N_runs);
        end
    end
%     switch StatsName
%         case 'SRE'
...
%----------------------------------------
function  GLNU =calculate_GLNU(tGLRLM,r_matrix,c_matrix,N_runs)
% Long Run High Gray-Level Emphasis (LRHGE):
%
term  = sum(tGLRLM).^2;
GLNU = sum(term)/N_runs;
%----------------------------------------
function  RLNU =calculate_RLNU(tGLRLM,r_matrix,c_matrix,N_runs)
% Long Run High Gray-Level Emphasis (LRHGE):
%
term  = sum(tGLRLM').^2;
RLNU = sum(term)/N_runs;
